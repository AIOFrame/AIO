...
$rotate = $ps['r'] ?? 0;
$padding = $ps['p'] ?? 4;
$color = !empty( $ps['c'] ) ? json_decode( $ps['c'], 1 ) : [];
$color = empty( $color ) ? [ 'r' => 255, 'g' => 255, 'b' => 255 ] : $color;
$bg_color = !empty( $ps['bg'] ) ? json_decode( $ps['bg'], 1 ) : [];
$bg_color = empty( $bg_color ) ? ['r'=>0,'g'=>0,'b'=>0] : $bg_color;
$transparent = $ps['tr'] ?? 0;
//$captcha = $ps['ca'] ?? 0;
//print_r( $ps );
//echo '<pre>';
