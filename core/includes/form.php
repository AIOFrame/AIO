    ...
    function __editable_data( array $data = [], string $remove = '' ): string {
        ...
        foreach( $data as $k => $v ){
            ...
            if( $k == 'id' ) {
                ...
                $final[ $k ] = !empty( $v ) ? ( !is_array( $v ) && $v != strip_tags( htmlspecialchars_decode( $v ) ) ? htmlspecialchars( $v ) : $v ) : ''; //!empty( $v ) && ( $v != strip_tags( htmlspecialchars_decode( $v ) ) ) ? htmlentities( $v ) : $v;
            }
        }
